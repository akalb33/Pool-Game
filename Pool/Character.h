...
public:
   ...
   // Checks if two polygons are within the specified distance from each other
   // Measured from the Origins of both polygons
   template <typename T>
   bool WithinProximity(std::vector<T*>& characters, float proximity)
   {
      bool found = false;
      characters.clear();
           ...
           it++)
      {
         if (((*it)->GetType() == typeid(T)) &&
             ...
             ((*it)->GetStatus() == alive))
         {
            float changeX = mX - (*it)->mX;
            float changeY = mY - (*it)->mY;
...
public:
   ...
   void UseSFMLScaling() { mUseSFMLScaling = true; }
